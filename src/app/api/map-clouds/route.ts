  ...
  try {
    ...
    const points = entries
      .map(({ item, originalIndex }) => {
        ...
        const values = Array.isArray(item?.hourly?.cloud_cover)
          ? item.hourly.cloud_cover
          : [];
        const times = Array.isArray(item?.hourly?.time)
          ? item.hourly.time
          : [];

        const cloudCover = values[hourlyIndex];
    ...
    if (!points.length) {
      return NextResponse.json(
        {
          error: "Cloud overlay has no usable samples.",
          ...responseMetadata(
            coordinates,
            batches,
            batchFailures,
    ...
    console.warn("Map cloud API failed:", error);

    return NextResponse.json(
      { error: "Unable to generate cloud map." },
      { status: 502 }
    );
  }
}
