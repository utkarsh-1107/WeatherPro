  ...
  try {
    ...
    const points = entries
      .map(({ item, originalIndex }) => {
        ...
        const values = Array.isArray(item?.hourly?.temperature_2m)
          ? item.hourly.temperature_2m
          : [];
        const times = Array.isArray(item?.hourly?.time)
          ? item.hourly.time
          : [];

        const temperature = values[hourlyIndex];
    ...
    if (!points.length) {
      return NextResponse.json(
        {
          error: "Temperature overlay has no usable samples.",
          ...responseMetadata(
            coordinates,
            batches,
            batchFailures,
    ...
    console.warn("Map temperature API failed:", error);

    return NextResponse.json(
      { error: "Unable to generate temperature map." },
      { status: 502 }
    );
  }
}
